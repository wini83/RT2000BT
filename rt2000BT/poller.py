...
def poll_valve(valve, client, topic: str | None = None) -> bool:
    ...
    payload = {
        ...
        "temperature": valve.current_temp,
        "setpoint": valve.set_point_temp,
        "mode": _mode_label(valve.mode_auto),
    }

    client.publish(f"{base_topic}/battery", payload=str(payload["battery"]), qos=0, retain=True)
    client.publish(f"{base_topic}/temperature", payload=str(payload["temperature"]), qos=0, retain=True)
    client.publish(f"{base_topic}/setpoint", payload=str(payload["setpoint"]), qos=0, retain=True)
